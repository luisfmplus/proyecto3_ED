...
void conectarServidor() {
	...
	if (iConnect == SOCKET_ERROR) {
		agregarArchivo("cliente", "Conexion fallo en puerto " + to_string(puerto) + " & Error No -> " + to_string(WSAGetLastError()) + "\n");
	}
	else {
		agregarArchivo("cliente", "Conexion exitosa en puerto " + to_string(puerto) + "\n");
	}
}
...
void enviarMensaje() {
	...
	if (iSend == SOCKET_ERROR) {
		...
		agregarArchivo("cliente", "Data Sending Success desde puerto: "+to_string(puerto)+"\n");
	}
}
...
void iniciarCliente() {
	...
	conectarServidor();
	recibirMensaje();
	puerto = StrToNum(string(RecvBuffer));
	cerrarSocket();
	limpiar();
	if (puerto != 0) {
		inicio();
